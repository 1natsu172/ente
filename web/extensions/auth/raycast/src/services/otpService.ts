...
import { AuthEntity, AuthKey } from "@ente/types/api";
import { Code } from "@ente/types/code.js";
...
export const getAuthCodes = async (): Promise<Code[]> => {
  ...
  try {
    ...
    const authenticatorKey = await cryptoWorker.decryptB64(authKeyData.encryptedKey, authKeyData.header, masterKey);
    ...
    const authCodes = await Promise.all(
      authEntity
        ...
        .map(async (entity) => {
          try {
            if (!entity.encryptedData || !entity.header) {
              throw Error("necessary code data missing.");
            }
            ...
            logError(Error("failed to parse code"), "codeId = " + entity.id);
            return null;
          }
        }),
    );
    // Remove null and undefined values
    const filteredAuthCodes = authCodes.filter((f): f is Code => f !== null && f !== undefined);
...
export const getAuthKey = async (): Promise<AuthKey> => {
  try {
    ...
    if (e instanceof ApiError && e.httpStatusCode === HttpStatusCode.NotFound) {
...
export const getDiff = async (sinceTime: number, limit = 2500): Promise<AuthEntity[]> => {
  try {
    const resp = await HTTPService.get(
